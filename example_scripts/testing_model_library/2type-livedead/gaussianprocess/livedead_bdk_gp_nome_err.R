...
# Add in the error
dat$type1_error <- round(sapply(dat$type1, function(x) rnorm(1, x, x * obs_err)))
dat$type2_error <- pmax(0, round(sapply(dat$type2, function(x) rnorm(1, x, x * obs_err))))
dat <- dat %>%
  rename(type1_noerror = type1, type2_noerror = type2) %>%
  rename(type1 = type1_error, type2 = type2_error)

dat %>% select(rep, time, concentration, log_conc, type1_noerror, type2_noerror) %>%
  pivot_longer(!c(rep, time, concentration, log_conc), names_to = "type") %>%
  ggplot(aes(x = time, group = interaction(type, as.factor(rep)), color = type)) +
  geom_line(aes(y = value)) +
  facet_wrap(concentration ~ .) +
...
stanfileloc <- system.file("model_library/2type-livedead/gaussianprocess/birthdeathclearance_error.stan", package = "bestdr")
