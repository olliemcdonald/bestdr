/*
Multitype Branching Process CLT Stan code template w/o mixed effects
...
transformed parameters{
  ...
  matrix[N,ntypes] z;
  array[nx, n_dt_unique] vector[ntypes*ntypes + ntypes*ntypes*ntypes] moments; //raw single-ancestor moments vector evolving over time
...
%s
      ...
      moments[xi] = ode_bdf_tol(moment_ode, init_state, 0, dt, 1e-6, 1e-6, 1000, ntypes, nevents, e_mat, r_mat);
    }
  }
}
model{
...
%s
  ...
  { // block to make these variables local
    array[nx, n_dt_unique] matrix[ntypes,ntypes] m_t; //fisrt moment matrices
    array[nx, n_dt_unique] matrix[ntypes,ntypes*ntypes] d_t; //second moments indexing goes (j,k,i)
    ...
    for(xi in 1:nx){
      for(ti in 1:n_dt_unique){
        m_t[xi,ti] = to_matrix(head(moments[xi,ti],ntypes*ntypes), ntypes,ntypes);
        d_t[xi,ti] = to_matrix(segment(moments[xi,ti],ntypes*ntypes+1, ntypes*ntypes*ntypes),ntypes,ntypes*ntypes);
      }
    }
    ...
    for(n in 1:N){
      ...
      mu_t = (m_t[xi,ti]')*to_vector(count_prev[n]);
      temp = to_matrix(count_prev[n]*d_t[xi,ti],ntypes,ntypes);
      for(i in 1:ntypes){
        sigma_t[i,i] = temp[i,i] - count_prev[n]*(col(m_t[xi,ti],i).*col(m_t[xi,ti],i)); //subtract to get covariance from 2nd moment
        for(j in 1:ntypes){
          sigma_t[i,j] = temp[i,j] - count_prev[n]*(col(m_t[xi,ti],i).*col(m_t[xi,ti],j)); //subtract to get covariance from 2nd moment
