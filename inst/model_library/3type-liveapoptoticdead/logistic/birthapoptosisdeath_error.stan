...
data{
  ...
  real prior_mu_a0;            // mean for prior distribution of a0 (left asymptote)
  real<lower=0> prior_s_a0;    // standard deviation for prior distribution of a0
  real prior_mu_da;            // diff of asymptotes
  real<lower=0> prior_s_da;    
  real prior_mu_a50;           // concentration at midpoint / inflection point
  real<lower=0> prior_s_a50;   
  real prior_mu_ah;            // slope at inflection point
  real<lower=0> prior_s_ah;
    ...
    for(c in 1:nc){
      ...
      else
      {
        r_mat[1, p_vec[1]] = bi + db / (1 + exp(bh * (conc[c] - b50)));
    		r_mat[2, p_vec[2]] = a0 + da - da / (1 + exp(ah * (conc[c] - a50)));
      }
      r_mat[3, p_vec[3]] = d;
      r_mat[4, p_vec[3]] = k;
...
model{
  ...
  a0 ~ normal(prior_mu_a0, prior_s_a0);
  da ~ normal(prior_mu_da, prior_s_da);
  a50 ~ normal(prior_mu_a50, prior_s_a50);
  ah ~ normal(prior_mu_ah, prior_s_ah);
