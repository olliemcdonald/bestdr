...
data{
  ...
  /*******************/
  // INSERT PRIORS INFO HERE
  // NEED TO MAKE A WAY TO INCLUDE CONSTRAINTS?
  // CAN AUTOMATE BASED ON PRIORS DEFINED IN MODEL SECTION
...
parameters{
  ...
  // NEED TO MAKE A WAY TO INCLUDE CONSTRAINTS?
  // PARAMETERS CAN BE FOUND FROM THE FUNCTION FORM AND MATCHED TO THE MODEL PRIORS
  // real<lower=0> bi;
  // real<lower=0> db;
  // real b50;
  // real<lower=0> bh;
...
transformed parameters{
  ...
  { // block to make these variables local
    ...
    for(c in 1:nc){
      ...
      /*******************/
      // INSERT FUNCTION CONNECTING RATES TO PARAMETERS HERE
      // PARAMETERS FOUND FROM PARAMETERS BLOCK
      // LHS r_mat[index, parent_vector[index]]
      // RHS function with parameters
      // r_mat[i, p_vec[i]] = function
...
model{
  ...
  { // block to make these variables local
    ...
    for(n in 1:N){
      ...
      count[n] ~ multi_normal(mu_t, sigma_t);
    }
  }
}