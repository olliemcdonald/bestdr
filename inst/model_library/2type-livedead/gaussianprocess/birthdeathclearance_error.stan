...
data{
  ...
  array[nc] vector[nd] conc;              // value of each concentration
  ...
  real mu_k;            // mean for prior distribution of k
  real<lower=0> s_k;    // standard deviation for prior distribution of k
...
transformed parameters{
  ...
  {  // Keep matrices local so they don't get stored
    ...
    matrix[nc, nc] K_b = gp_exp_quad_cov(conc, alpha_b, rho_b) + diag_matrix(rep_vector(1e-10, nc));
    matrix[nc, nc] L_K_b = cholesky_decompose(K_b);
    matrix[nc, nc] K_d = gp_exp_quad_cov(conc, alpha_d, rho_d) + diag_matrix(rep_vector(1e-10, nc));
