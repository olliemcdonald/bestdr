...
dat %>% select(c(rep, time, concentration, log_conc, type1, type2)) %>%
  pivot_longer(!c(rep, time, concentration, log_conc), names_to = "type") %>%
  ggplot(aes(x = time, group = interaction(type, as.factor(rep)), color = type)) +
  geom_line(aes(y = value)) +
  facet_wrap(concentration ~ .) +
...
stanfileloc <- system.file("model_library/2type-livedead/logistic/birthdeathclearance_error.stan", package = "bestdr")
...
params_posterior <- model_posterior$draws(variables = c("bi", "db", "b50", "bh", "d0", "dd", "d50", "dh", "k", "obs_err"), format = "df")
...
params_posterior_l <- params_posterior %>%
  select(.chain, bi, db, b50, bh, d0, dd, d50, dh, k, obs_err) %>%
