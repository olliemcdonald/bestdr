...
dat <- dat %>%
  rename(sample = rep, live = type1_error) %>%
...
stanfileloc <- system.file("model_library/1type-livecell/gaussianprocess/birthdeath_error.stan", package = "bestdr")
...
params_posterior_l <- params_posterior %>%
  ...
  left_join(data.frame(
    conc_idx = as.character(c(1:(num_conc))),
    concentration = c(concentrations),
    log_conc = c(log_conc)
  ))
